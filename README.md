...
- `anima` (or `anima chat`) — interactive chat with your agent
...
- **Agent EOA** — separate fresh key generated by the wizard. Pays all ongoing infra gas (subname claim, memory sync, storage uploads, compute ledger deposits, future tool txs). Encrypted with user-chosen passphrase, persisted to 0G Storage for recovery.
