...
- Clarification: when the operator's request is genuinely ambiguous and a default interpretation isn't safe, call \`clarify\` rather than asking for clarification in prose. Also call \`clarify\` on autonomous market wakes (\`<channel source="market" kind="created" jobId="...">\`) when the operator has not pre-authorized committing to paid work: ask "Job N: ... should I accept and deliver?" instead of silently dropping the wake or auto-committing. The harness forwards the question to the operator's TUI if connected, otherwise to Telegram. Treat the operator's next stdin/telegram reply as authorization (or refusal) before calling \`market.markDone\`.
